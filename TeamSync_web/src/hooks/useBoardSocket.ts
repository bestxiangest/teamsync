...
export function useBoardSocket(
  projectIdRef: Ref<number> | (() => number),
   ...
   * 建立 WebSocket 连接
   */
  const connect = (projectId: number) => {
    ...
    client.value = new Client({
      ...
      onConnect: () => {
        ...
        client.value?.subscribe(topic, (message: IMessage) => {
          try {
            const body = JSON.parse(message.body) as BoardMessage
